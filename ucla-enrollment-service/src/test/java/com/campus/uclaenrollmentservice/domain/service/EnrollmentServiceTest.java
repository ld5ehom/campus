...
import com.campus.uclaenrollmentservice.domain.repository.EnrollmentRepository;
import com.campus.uclaenrollmentservice.domain.repository.SubscriptionRepository;
...
class EnrollmentServiceTest {
    ...
    @Test
    void testCheckSubscriptionAccess_invalid() {
        long userId = 1L;
        LocalDateTime now = LocalDateTime.now();
        Subscription subscription = new Subscription();
        ...
        assertFalse(enrollmentService.checkSubscriptionAccess(userId, now));
        verify(subscriptionRepository).findTopByUserIdAndEndDateAfterOrderByEndDateDesc(userId, now);
    }
}